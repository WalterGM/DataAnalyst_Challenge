...
if __name__ == "__main__":
    ...
    config = ConfigParser()
    config.read(config_file)v

    basePath = config['URLS'].get('BasePath')
    files = config['FILE_NAMES'].get("files").split(",")
    engine = create_engine('postgresql://postgres:admin@localhost:5432/challenge')
    ...
    for file in files:
        ...
        df_data = df_data.rename(columns=new_columns)[config['COLUMNS'].get(file).split(",")]
